...
@section('title') Drivers Reports Search Result @endsection
...
@section('content')
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Drivers Reports Search Result: {{ $keyword }}
      </h1>
      <ol class="breadcrumb">
        ...
        <table class="table table-hover">
          ...
          <tbody>
            @foreach($users as $user)
              @if(count($user->report) > 0 && $user->user_type == 2)
                @foreach($user->report as $report)
                  <tr>
                    ...
                    <td>
                      <a href="{{ route('admin.view.driver.details', ['id' => $report->complainant->id]) }}">{{ ucwords($report->complainant->first_name . ' ' . $report->complainant->last_name) }}</a>
                    </td>
                    <td>
